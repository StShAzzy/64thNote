...
int DMAUsed;
const BYTE ZEROMON=0;

__inline const BYTE * PageROM(DWORD addr) {
	return (ROMPages[addr/0x100000])?ROMPages[addr/0x100000]+(addr%0x100000):&ZEROMON;
}
...
void PI_DMA_WRITE (void) {
	...
	//DisplayError("PI_DMA RDRAM=%#08x CART=%#08x",PI_DRAM_ADDR_REG,PI_CART_ADDR_REG);
	PI_STATUS_REG |= PI_STATUS_DMA_BUSY;
	...
	if ( PI_CART_ADDR_REG >= 0x08000000 && PI_CART_ADDR_REG <= 0x08010000) {
		//DisplayError("Attempted read from save");
		return;
	}
	...
	if ( PI_CART_ADDR_REG >= 0x10000000 && PI_CART_ADDR_REG <= 0x1FBFFFFF) {
		...
		MI_INTR_REG |= MI_INTR_PI;
		CheckInterrupts();
		// I suppose this was an attempt to simulate the time delay of PI DMA
