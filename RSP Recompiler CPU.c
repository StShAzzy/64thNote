...
DWORD RSPCompilePC, RSPBlockID = 0;
...
void ReOrderInstructions(DWORD StartPC, DWORD EndPC) {
	...
	PreviousOp.Hex = *(DWORD*)(IMEM + StartPC);
	...
	for (Count = 0; Count < InstructionCount; Count += 4) {
		CurrentPC = StartPC;
		PreviousOp.Hex = *(DWORD*)(IMEM + CurrentPC);
		ReorderedOps = 0;

		for (;;) {
			CurrentPC += 4;
			if (CurrentPC >= EndPC) { break; }
			CurrentOp.Hex = *(DWORD*)(IMEM + CurrentPC);

			if (TRUE == CompareInstructions(CurrentPC, &PreviousOp, &CurrentOp)) {
				/* Move current opcode up */
				*(DWORD*)(IMEM + CurrentPC - 4) = CurrentOp.Hex;
			 	*(DWORD*)(IMEM + CurrentPC) = PreviousOp.Hex;
...
void LinkBranches(RSP_BLOCK * Block) {
	...
	for (Count = 0; Count < RSPCurrentBlock.ResolveCount; Count++) {
		...
		RSPx86_SetBranch32b(JumpWord, (DWORD*)X86Code);
...
void CompilerRSPBlock ( void ) {
	...
	if (X86BaseAddress & 7) {
		...
		for (Count = 0; Count < Padding; Count++) {
			CPU_Message("%08X: nop", RSPRecompPos);
			*(RSPRecompPos++) = 0x90;
		}
	}

	CPU_Message("====== block %d ======", RSPBlockID++);
	CPU_Message("x86 code at: %X",RSPRecompPos);
	CPU_Message("Jumpt Table: %X",Table );
	CPU_Message("Start of Block: %X",RSPCurrentBlock.StartPC );
	...
	*(RSPJumpTable + (RSPCompilePC >> 2)) = RSPRecompPos;
	...
	do {
		...
		**/
		...
		if (FALSE == IsOpcodeNop(RSPCompilePC)) {
