...
void CheckWaitMode(void) {
	//if (Timers.CurrentTimerType==ViTimer && WaitMode) Timers.Timer=-1;
	if (WaitMode) Timers.Timer=-1;
	//DisplayError("CheckWaitMode");
}
...
void CompileSystemCheck (DWORD TimerModifier, DWORD TargetPC, REG_INFO RegSet) {
	...
	// Timer
	Call_Direct(CheckWaitMode,"CheckWaitMode");
	if (TimerModifier != 0) {
	...
	do {
		...
		case R4300i_SPECIAL:
			...
			//case R4300i_SPECIAL_BREAK:
				...
				Section->CompilePC -= 4;
				break;
			case R4300i_SPECIAL_BREAK: break; // nothing special
